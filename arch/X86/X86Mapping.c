...
static insn_map x86insns[] = {	// full x86 instructions
	// dummy item
	{
		0, 0,
#ifndef CAPSTONE_DIET
...
{
	int i = insn_find(x86insns, ARR_SIZE(x86insns), id, &h->insn_cache);
	if (i != 0) {
		insn->id = x86insns[i].mapid;

		if (h->detail) {
			memcpy(insn->detail->regs_read, x86insns[i].regs_use, sizeof(x86insns[i].regs_use));
			insn->detail->regs_read_count = (uint8_t)count_positive(x86insns[i].regs_use);

			// special cases when regs_write[] depends on arch
			switch(id) {
				default:
					memcpy(insn->detail->regs_write, x86insns[i].regs_mod, sizeof(x86insns[i].regs_mod));
					insn->detail->regs_write_count = (uint8_t)count_positive(x86insns[i].regs_mod);
					break;
				case X86_RDTSC:
			...
			memcpy(insn->detail->groups, x86insns[i].groups, sizeof(x86insns[i].groups));
			insn->detail->groups_count = (uint8_t)count_positive(x86insns[i].groups);

			if (x86insns[i].branch || x86insns[i].indirect_branch) {
				// this insn also belongs to JUMP group. add JUMP group
				insn->detail->groups[insn->detail->groups_count] = X86_GRP_JUMP;
				insn->detail->groups_count++;
			}

			switch (x86insns[i].id) {
				case X86_OUT8ir:
				case X86_OUT16ir:
				case X86_OUT32ir:
...
{
	unsigned int id;
	int i = insn_find(x86insns, ARR_SIZE(x86insns), opcode, &h->insn_cache);
	if (i != 0) {
		id = x86insns[i].mapid;
		switch(id) {
			default:
